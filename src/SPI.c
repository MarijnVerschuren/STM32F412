...
static _FINLINE void SPI_IRQ_handler(SPI_t* spi, io_buffer_t* io) {
	if (!io) { return; }
	if (spi->SR & 0x00000001UL && io->ien && io->ptr) {
	...
	if (spi->SR & 0x00000002UL && io->oen && io->ptr) {
...
void SPI1_handler() {
	SPI_IRQ_handler(SPI1, SPI_buffer[0]);
}
 ...
 * IRQ
 * */
void start_SPI_read8_IRQ(SPI_t* spi, io_buffer_t* buffer) {
	spi->CR2 |= 0x00000040UL;				// enable RXNE interrupt
	register uint32_t irqn = 0;
	switch ((uint32_t)spi) {
	case SPI1_BASE:	irqn = SPI1_IRQn; break;
	case SPI2_BASE:	irqn = SPI2_IRQn; break;
	case SPI3_BASE:	irqn = SPI3_IRQn; break;
	case SPI4_BASE:	irqn = SPI4_IRQn; break;
	case SPI5_BASE:	irqn = SPI5_IRQn; break;
	}
	NVIC_set_IRQ_priority(irqn, 0);
